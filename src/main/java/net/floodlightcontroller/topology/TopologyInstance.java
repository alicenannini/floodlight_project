...
public class TopologyInstance {
    ...
    protected class PathCacheLoader extends CacheLoader<RouteId, List<Route>> {
        TopologyInstance ti;
    ...
    protected class NodeDist implements Comparable<NodeDist> {
        ...
        public boolean equals(Object obj) {
            ...
            if (node == null) {
                if (other.node != null)
                    return false;
            } else if (!node.equals(other.node) || dist != other.dist)
                return false;
            return true;
        }
    ...
    protected void calculateBroadcastNodePortsInClusters() {
        ...
        for(Cluster c: clusters) {
            ...
            Map<DatapathId, List<Link>> links = tree.getLinks();
            if (links == null) continue;
            for(DatapathId nodeId: links.keySet()) {
                List<Link> l = links.get(nodeId);
                if (l == null) continue;
                /* TODO */
                NodePortTuple npt1 = new NodePortTuple(l.get(0).getSrc(), l.get(0).getSrcPort());
                NodePortTuple npt2 = new NodePortTuple(l.get(0).getDst(), l.get(0).getDstPort());
                nptSet.add(npt1);
                nptSet.add(npt2);
            }
    ...
    protected List<Route> buildroute(RouteId id) {
        NodePortTuple npt;
    ...
    protected boolean routeExists(DatapathId srcId, DatapathId dstId) {
        ...
        List<Link> link = bt.getLinks().get(srcId);
